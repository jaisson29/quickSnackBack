...
CREATE TABLE IF NOT EXISTS `quickSnack`.`perfil` (
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`dominio` (
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`valor` (
  `valorId` INT NOT NULL AUTO_INCREMENT,
  `param` VARCHAR(255) NOT NULL,
  `domId` INT NOT NULL,
  PRIMARY KEY (`valorId`),
  INDEX `valueXDomain_idx` (`domId` ASC) ,
  CONSTRAINT `valueXDomain`
    FOREIGN KEY (`domId`)
    REFERENCES `quickSnack`.`dominio` (`domId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`usuario` (
  ...
  CONSTRAINT `userXProfile`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `genderXValue`
    FOREIGN KEY (`usuGen`)
    REFERENCES `quickSnack`.`valor` (`valorId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `docTypeXValue`
    FOREIGN KEY (`usuTipoDoc`)
    REFERENCES `quickSnack`.`valor` (`valorId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`pagina` (
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`perxpag` (
  ...
  CONSTRAINT `profileXPage`
    FOREIGN KEY (`perfilId`)
    REFERENCES `quickSnack`.`perfil` (`perfilId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `pageXProfile`
    FOREIGN KEY (`paginaId`)
    REFERENCES `quickSnack`.`pagina` (`paginaId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`categoria` (
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`producto` (
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`transaccion` (
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`proveedor` (
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`compra` (
  `compraId` INT NOT NULL AUTO_INCREMENT,
  `provId` INT NOT NULL,
  `fechaCompra` DATE NOT NULL,
  PRIMARY KEY (`compraId`),
  INDEX `purchBillXProvider_idx` (`provId` ASC) ,
  CONSTRAINT `purchBillXProvider`
    FOREIGN KEY (`provId`)
    REFERENCES `quickSnack`.`proveedor` (`provId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`detVenta` (
  ...
  CONSTRAINT `saleDetXProduct`
    FOREIGN KEY (`prodId`)
    REFERENCES `quickSnack`.`producto` (`prodId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `saleDetXSaleBill`
    FOREIGN KEY (`transacId`)
    REFERENCES `quickSnack`.`transaccion` (`transacId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `quickSnack`.`detCompra` (
  ...
  CONSTRAINT `productXPurchDet`
    FOREIGN KEY (`prodId`)
    REFERENCES `quickSnack`.`producto` (`prodId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `comXDetCom`
    FOREIGN KEY (`compraId`)
    REFERENCES `quickSnack`.`compra` (`compraId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
START TRANSACTION;
USE `quickSnack`;
INSERT INTO `quickSnack`.`perfil` (`perfilId`, `perfilNom`) VALUES (1, 'Administrador');
INSERT INTO `quickSnack`.`perfil` (`perfilId`, `perfilNom`) VALUES (2, 'Usuario');
INSERT INTO `quickSnack`.`perfil` (`perfilId`, `perfilNom`) VALUES (3, 'Cajero');
...
START TRANSACTION;
USE `quickSnack`;
INSERT INTO `quickSnack`.`valor` (`valorId`, `param`, `domId`) VALUES (1, 'masculino', 1);
INSERT INTO `quickSnack`.`valor` (`valorId`, `param`, `domId`) VALUES (2, 'femenino', 1);
...
INSERT INTO `quickSnack`.`usuario` (`usuId`, `usuTipoDoc`, `usuNoDoc`, `usuGen`, `usuNom`, `usuEmail`, `usuContra`, `usuIngreso`, `usuImg`, `perfilId`, `usuFecha`, `usuPassCode`) VALUES (3, 4, '987654', 2, '', 'miguel@outllok.com', '12349', '2023-07-23 12:29:34', NULL, 3, NULL, NULL);
...
INSERT INTO `quickSnack`.`pagina` (`paginaId`, `paginaNom`, `paginaIcon`, `paginaRuta`) VALUES (9, 'Compra', 'fa-cart-shopping', '/compra');
